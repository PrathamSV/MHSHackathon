...
def play():
    ...
    if x == 5:
        ...
        y = "0"
        while y != "1" and y != "2":
            input("Discard one or both cards (Select a number from 1-2):")
        if y == "1":
            z = "0"
            while z != "1" and z != "2":
                input("What card do you want to discard? (Select a number from 1-2): ")
            if turn % 2 == 1:
        ...
        if turn % 2 == 1:
            hand2.pop(random.randint(0, 2))
            hand2.append(draw())
        else:
            hand1.pop(random.randint(0, 2))
            hand1.append(draw())
    if turn % 2 == 1:
        hand1.append(draw())
    else:
